...
export default function AnalyticsContent() {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      // TODO: redraw to the new monogram — yellow rounded tile #FFE500 with
      ...
      for (let p = 1; p <= totalPages; p++) {
        ...
        pdf.setFontSize(7)
        pdf.setFont('helvetica', 'normal')
        pdf.text('Generated by Thrive', MARGIN, 292.5)
        pdf.text(`${reportDate}  •  Page ${p} of ${totalPages}`, W - MARGIN, 292.5, { align: 'right' })
      }

      // Save
      const filename = `Thrive-Analytics-${companyName.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().split('T')[0]}.pdf`
      pdf.save(filename)
    } catch (err) {
  ...
  const handleExportCsv = useCallback(() => {
    ...
    try {
      ...
      const rows: string[] = []
      rows.push(`Thrive Analytics Report — ${companyName} — ${dateLabel}`)
      rows.push('')
      ...
      a.href = url
      a.download = `Thrive-Analytics-${companyName.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().split('T')[0]}.csv`
      a.click()
  ...
  const handleExportXlsx = useCallback(async () => {
    ...
    try {
      ...
      const filename = `Thrive-Analytics-${companyName.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().split('T')[0]}.xlsx`
      ...
      <div className={`${styles.container} ${!isProfessional ? styles.blurredContent : ''}`}>
        ...
        <div className={`${styles.sectionCard}`} style={{ marginBottom: '1.5rem' }}>
          ...
          {retentionData && retentionData.totalHired > 0 ? (
            ...
            <div className={styles.chartEmpty}>
              <div style={{ textAlign: 'center' }}>
                <p style={{ margin: '0 0 0.25rem', color: '#94a3b8' }}>No hires yet — retention tracking begins once you hire your first candidate through Thrive</p>
