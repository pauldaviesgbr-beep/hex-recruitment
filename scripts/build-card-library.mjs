...
// THE ?ref TAG IS WHAT MAKES AN ARRIVAL ATTRIBUTABLE, AND THESE VALUES ARE
// MEASURED RATHER THAN CHOSEN.
//
// Every one was run through the real `normalizeSource` from lib/attribution.ts
// on 21 Sept 2026 — not read off the alias table, because the table is only
// half the rule. What each resolves to:
//
//     ?ref=li         -> LinkedIn    basis: tag
//     ?ref=ig         -> Instagram   basis: tag
//     ?ref=ig-story   -> Instagram   basis: tag   (raw ref kept for drill-down:
//                                    normalizeSource keys off the prefix before
//                                    the first '-', which is the documented
//                                    fb-chefsuk -> Facebook pattern)
//     ?ref=fb         -> Facebook    basis: tag
//     ?ref=tt         -> TikTok      basis: tag
//
// X/TWITTER IS DELIBERATELY ABSENT. There is no alias for it: `?ref=x` resolves
// to the channel "X", while the REFERRER path resolves x.com, twitter.com and
// t.co to "X/Twitter". Emitting ?ref=x would split one channel across two
// labels in every chart — the exact thing the referrer mapping says it exists
// to prevent. Adding the alias is a one-line change to lib/attribution.ts,
// which is a deployed file and therefore somebody's decision, not this
// script's. Until then an X post carries the untagged link and its arrivals
// land on the referrer, which still reads X/Twitter correctly.
const PLATFORM_TAGS = [
  ['LinkedIn', 'li'],
  ['Instagram — feed', 'ig'],
...
// THE ONE TEST FOR "IS THIS CAPTION FILE OURS", USED BY BOTH THE WRITE PATH AND
// THE DELETE PATH — because it was wrong in one of them the first time.
//
// THE MANIFEST IS NOT EVIDENCE. It recorded `Sous Chef - Agency - posts.md` as
// the library's on the first run, because that run had overwritten a
// hand-written file of that name. The file was restored from Drive afterwards
// and the manifest still claims it. A guard keyed on the manifest would destroy
// it again, and this time with no version history to recover from.
//
// So the FILE has to say it. Every caption this script writes ends with that
// sentence and a human-written one does not. Absent means ours to create.
const GENERATED_MARKER = 'Generated by `scripts/build-card-library.mjs`'
function captionIsOursOrAbsent(file) {
...
function captionFor(job, entry, sentences) {
  ...
  L.push('---')
  L.push('')
  L.push('## Links — ONE PER PLATFORM, AND THEY ARE NOT INTERCHANGEABLE')
  L.push('')
  L.push('Paste the link for the platform you are posting on. The `?ref` tag is the')
  L.push('only thing that makes the arrival attributable: it stamps the signup as a')
  L.push('DECLARED source rather than a guess from a referrer header, which is why')
  L.push('13 of last month’s 31 signups can be read as LinkedIn at all.')
  L.push('')
  for (const [platform, tag] of PLATFORM_TAGS) {
    L.push(`- **${platform}** — \`${url}?ref=${tag}\``)
  }
  L.push('')
  L.push('**X/Twitter has no tag yet.** `?ref=x` would record the channel as "X" while')
  L.push('a referrer from x.com records "X/Twitter", splitting one channel across two')
  L.push('labels. Until an alias exists in `lib/attribution.ts`, post the bare link and')
  L.push('let the referrer carry it:')
  L.push('')
  L.push(`- **X/Twitter** — \`${url}\``)
  L.push('')
  L.push('---')
  L.push('')
...
async function main() {
  ...
  for (const job of jobs) {
    ...
    if (FORCE && manifest[job.id]) {
      ...
      // A CAPTION IS ONLY DELETED IF IT PROVES IT IS OURS, AND THE MANIFEST
      // SAYING SO IS NOT PROOF.
      //
      // The manifest recorded `Sous Chef - Agency - posts.md` as the library's
      // on the first run — because the first run had overwritten a hand-written
      // file of that name. That file has since been restored from Drive, and
      // the manifest still claims it. Trusting the manifest here would destroy
      // it a second time, and this time there would be no version history left
      // to restore from because the delete takes it to the bin.
      //
      // So the file itself has to say it was generated. Every caption this
      // script writes ends with that sentence; a human-written one does not.
      const oldCaption = manifest[job.id].captionFile
      if (oldCaption && fs.existsSync(oldCaption)) {
        if (captionIsOursOrAbsent(oldCaption)) {
          fs.rmSync(oldCaption)
        } else {
          console.log(`      NOT DELETED — this caption is not ours, whatever the manifest says:`)
    ...
    // THE CAPTION NAME CARRIES THE JOB ID BECAUSE THE CARD'S NAME DOES.
    //
    ...
    // jobs. Naming a caption from the title alone made later roles overwrite
    // earlier ones — 8 captions lost on the first full run, 21 Sept 2026.
    //
    // The id is NOT appended here. It arrives inside `stem`, which is read back
    // off the filename the generator actually wrote, so the caption and its
    // cards cannot end up with different ids the way two copies of one rule
    ...
    // have chosen, and the first run destroyed it.
    //
    // THE FIRST VERSION OF THIS GUARD ASKED THE MANIFEST AND WAS USELESS — the
    // manifest had already recorded that very file as the library's, so the
    // check passed and the write went through. Watched on 21 Sept 2026: a
    // planted 66-byte hand-written caption was replaced by 3,128 bytes of
    // generated text while the DELETE guard beside it correctly refused. Half a
    // fix reads exactly like a whole one.
    if (!captionIsOursOrAbsent(captionFile)) {
