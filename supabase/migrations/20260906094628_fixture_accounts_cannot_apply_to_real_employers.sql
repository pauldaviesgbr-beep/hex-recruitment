...
-- there is no server-side place to put this except here.
...
comment on table public.fixture_accounts is
  'Test and review accounts. A fixture CANDIDATE may not apply to an advert owned by anyone who is not a fixture EMPLOYER. See fixture_application_refusal().';
...
-- symmetrical: MISSING a fixture leaves things exactly as they are today, while
-- WRONGLY INCLUDING a real person stops them applying for work.
...
-- THE DECISION, AS ITS OWN FUNCTION. Separate from the trigger ON PURPOSE, so
-- the exact decision that runs can be asked about ANY pair read-only -
-- including a real candidate against a real advert - without inserting a row.
-- Returns the refusal reason, or NULL to allow.
--
-- SECURITY DEFINER IS LOAD-BEARING. fixture_accounts has RLS; if the lookup ran
-- as the caller and a policy hid a row, the guard would silently decide "not a
-- fixture" and let the application through - failing OPEN and uselessly.
...
begin
  if public.fixture_application_refusal(new.candidate_id, new.job_id) is not null then
    ...
    -- sentence. A raw server string must never reach a person.
